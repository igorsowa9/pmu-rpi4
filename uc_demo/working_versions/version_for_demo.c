...
void send_phasor(double f1, double m1, double a1, double r1, long long int ptt, long long int ts_usec_start, long long int ts_usec, int phasor_counter, MQTTClient client, MQTTClient_message pubmsg, MQTTClient_deliveryToken token) {
        char frame[2000];
	long long int processed_ts_usec;
	float sec_estim;
	long long int ts_diff;

	/* estimation of the timestamp in order to synchronize RTDS vPPS with real world time*/
	sec_estim = floor(phasor_counter/(80000/128))+1;
	processed_ts_usec = round(ts_usec_start/1000000)*1000000 + sec_estim*1000000 + ptt;
	ts_diff = processed_ts_usec-ts_usec;
	if(ts_diff<-1e6){processed_ts_usec = processed_ts_usec + 1e6;}

        //sprintf(frame, "now ts: %lld \t processed_ts_usec: %lld \t diff: %lld \t ptt: %lld \t angle: %f \t ph.count: %d", ts_usec, processed_ts_usec, ts_diff, ptt, a1, phasor_counter);
	/*-----------------------------------------------------------------------------------*/
	...
	"{\"component\": \"BUS1\","
        ...
        "{\"component\": \"BUS1\","
            ...
            "\"phase\": \"C\","
            "\"data\": %f}"
        "]}", processed_ts_usec, m1,a1,f1,r1, m1,a1+120,f1,r1, m1,a1-120,f1,r1);

        pubmsg.payload = frame;
        pubmsg.payloadlen = strlen(frame);//12; //sizeof(double); //strlen(PAYLOAD);
...
int main(void)
{
	...
	conn_opts.ssl = &sslopts;
        ...
        conn_opts.ssl->trustStore = CLIENT_KEY_FILE;
	...
	// Interpolation
	double B, alpha, delta_bin, time_win, delta_f, f_estim, mem_f_estim, rocof_estim, A_estim, ph_estim;
	...
	long long int timestamp_usec_start, timestamp_usec_stop, timestamp_usec_first_slide;
		...
		while(status == SS_RUNNING && err == ERR_NO_ERROR && !enter_press() )
		{
			...
			if(index >= 0)
			{
				...
				*/
				if(first_pps_detect == 1 && transferStatus.currentTotalCount != mem_totalcount && (int)transferStatus.currentScanCount % (int)samplesPerChannel_checked == 0){
					...
					if ((int)transferStatus.currentTotalCount-(int)mem_totalcount != n_channel*samplesPerChannel_checked) {
						if (first_pps_detect_diff == 0) {
							...
							send_phasor(0,0,0,0,0,0,0,0, client, pubmsg, token);
						}
					}
					/*----------------------------------------------------------------*/
					...
					if(totalCount_updated_from_firstPPS < periods_for_calc*samples_in_period*n_channel) { // 3*rate/50
						...
						/* first slide timestamp for fake-timetag. */
                                                if (!gettimeofday(&timer_usec, NULL)) {
                                                timestamp_usec_first_slide = ((long long int) timer_usec.tv_sec) * 1000000ll + (long long int) timer_usec.tv_usec;
                                                }
                                                else {
                                                timestamp_usec_first_slide = -1;
                                                }
					}
				...
				rocof_estim = (mem_f_estim - f_estim)*(rate/samplesPerChannel_checked); // the most precise rocof (end most valatile) - rescaled to Hz/s
				...
				if (expected_frame_flag==1) {
					...
					/* absolute timestamp to compare delays */
					if (!gettimeofday(&timer_usec, NULL)) {
					...
					printf("Original ph_estim: %f Row timestamp in us: %lld\n", ph_estim, timestamp_usec);
	                                send_phasor(f_estim, A_estim, ph_synch*180/M_PI, rocof_estim, (long long int)expected_frame_ts, timestamp_usec_start, timestamp_usec, phasor_counter, client, pubmsg, token);
				}
				}
			}
