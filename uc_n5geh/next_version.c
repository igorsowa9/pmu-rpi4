...
/* SETTINGS */
#define SET_FR	    10 // reporting rate
...
int main(void)
{
	...
	int lowChan = 0;
	int highChan = 1; /* this is for fixed single channel i.e. maximum accuracy */
	...
	B = 1151.5; //1199.5/1151.5 // sum of Hann windowing for 2400 i.e. 3*40k/50 (check in matlab: sum(hann(2400)) )
		...
		while(status == SS_RUNNING && err == ERR_NO_ERROR && !enter_press() )
		{
			...
			if(index >= 0)
			{
				...
				*/
				...
				/* sort whether publish or igore depending on reporing rate*/
				if (ptt_ch1<0) {
